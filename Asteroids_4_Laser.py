...
class Spaceship(SpaceObject):
    ...
    """
    def shoot(self):
        laser_img = pyglet.image.load("Assetss\PNG\Lasers\laserBlue01.png")
        laser_img.anchor_x = laser_img.width //2
        laser_img.anchor_y = laser_img.height //2
        laser = Laser(laser_img, self.sprite.x + (0.5*self.radius*math.cos(self.rotation)) ,self.sprite.y +  (0.5*self.radius*math.sin(self.rotation)),  math.cos(self.rotation) * LASERSPEED ,  math.sin(self.rotation) * LASERSPEED, self.rotation)
...
class Asteroid(SpaceObject):
    ...
    def hit_by_laser(self, laser):
        ...
        laser.delete_laser(laser=laser)
        self.delete()
...
class Laser(SpaceObject):
    "Konśtruktor"
    def __init__(self, sprite, x, y, speed_x, speed_y, rotation):
        super().__init__(sprite,x,y,speed_x,speed_y)
        self.rotation = rotation
        pyglet.clock.schedule_once(self.delete_laser, LASERLIFETIME, laser=self)
    ...
    def delete_laser(self, laserlifetime=0, laser=None):
        if laser in game_objects:
            laser.delete()
"""
GAME WINDOW CLASS
"""
